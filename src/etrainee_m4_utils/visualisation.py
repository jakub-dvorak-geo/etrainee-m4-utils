...
ds_name_values = ('bila_louka', 'lucni_hora', 'pavia_centre')
...
def confusion_matrix(reference: np.ndarray, predict: np.ndarray,
                     ds_name: str = 'bila_louka') -> None:
    """Plots a confusion matrix. Uses sklearn.metrics.confusion_matrix() and
    matplotlib.
    ...
    Returns:
        None
    """
    conf_m = conf_matrix(reference, predict)

    fig, ax = plt.subplots(figsize=(7.5, 7.5))
    ax.matshow(conf_m, cmap=plt.cm.hot_r, alpha=0.3)
    # Add numbers to each position
    for i in range(conf_m.shape[0]):
        for j in range(conf_m.shape[1]):
            ax.text(x=j, y=i, s=conf_m[i, j], va='center', ha='center')
    ...
    _, classnames = _create_colorlist_classnames(ds_name=ds_name)
    classnames = classnames[np.unique(predict)[0]:]
